...
- 🔍 **Filter** resources with complex conditions
- 📊 **Sort** results by multiple fields
- 📦 **Include** related models (eager loading)
...
```php
// ✅ Good: Explicit allowed filters
->allowedFilters(['name', 'price'])

// ❌ Bad: Allow all filters (security risk)
// Don't do this
```
...
- **[API Reference →](/api/rest)** - See all API endpoints with Query Builder
- **[Testing →](/extending/backend#testing)** - Test API queries
