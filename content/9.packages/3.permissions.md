...
- 🔐 **Assign roles** to users (Admin, Manager, Customer, etc.)
- ✅ **Grant permissions** to roles or directly to users
- 🛡️ **Protect routes and actions** with middleware
- 🎯 **Check permissions** in controllers and views
- 📊 **Model-level permissions** for fine-grained control
  ...
  <div class="role-manager">
    ...
    <div class="current-roles">
      ...
      <div v-for="role in user.roles" :key="role.id" class="role-badge">
        {{ role.name }}
        <button @click="removeRole(role.name)">×</button>
      </div>
    </div>
...
```php
// ✅ Good: Assign role with permissions
$user->assignRole('admin');

// ❌ Bad: Assign each permission individually
...
- **[API Authentication →](/packages/sanctum-advanced)** - Secure API endpoints
- **[Admin Panel →](/admin)** - Role management UI
- **[Testing →](/extending/backend#testing)** - Test permissions
