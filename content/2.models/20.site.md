...
title: Site
description: Site model reference
...
public function setDefaultCurrencyAttribute($value)
{
    ...
    if (!in_array($value, config('cartino.currencies'))) {
...
```php
use Cartino\Events\SiteCreated;
use Cartino\Events\SiteUpdated;
use Cartino\Events\SitePublished;
