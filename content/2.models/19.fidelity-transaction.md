...
title: FidelityTransaction
description: FidelityTransaction model reference
...
{
    $pointsPerCurrency = config('cartino.fidelity.points_per_currency', 100);
...
{
    $expiryMonths = config('cartino.fidelity.points_expiry_months', 12);
...
```php
use Cartino\Events\PointsEarned;
use Cartino\Events\PointsRedeemed;
use Cartino\Events\PointsExpiring;
...
```php
use Cartino\Jobs\ExpirePointsJob;
